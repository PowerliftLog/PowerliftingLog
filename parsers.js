...
function parseWorkbook(wb){
  ...
  if (fmt === 'A') return parseA(wb);
  if (fmt === 'E') return parseE(wb);
  if (fmt === 'F') return parseF(wb);
  if (fmt === 'G') return parseG(wb);
  if (fmt === 'C') return parseCAutoFormat(wb);
  if (fmt === 'D') return parseD(wb);
  return parseB(wb);
}
